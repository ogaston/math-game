...
class Quiz extends React.Component {
  ...
  generateAnotherProblem = () => {
    ...
    const problem = [this.getRandomNumbers()];

    for (let i = 0; i < level; i += 5) {
      let symb = this.getRandomSymbol();
      let num =
        symb == "/" || symb == "*"
          ? this.getRandomNumbers(10)
          : this.getRandomNumbers();
      problem.push(symb, num);
    }

    const newProblem = problem.join(" ");
    this._isMounted &&
      this.setState({
        problem: newProblem
      });
  ...
  getRandomNumbers = num => {
    return (Math.random() * (num || 100)).toFixed();
  };
